...
export function createReplyActionHandler(
    ...
    return async function replyAction(
        ...
        const replyData = {
            ...
            threadRoot:
                parentPost.threadRoot != null &&
                typeof parentPost.threadRoot === 'number'
                    ? await postRepository.getById(parentPost.threadRoot)
                    : parentPost,
        };

        const newReply = Post.createFromData(account, replyData);
        await postRepository.save(newReply);
        await postRepository.updateReplyCount(parentPost.id!, true);
