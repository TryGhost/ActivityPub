...
export class PostController {
    ...
    async handleDerepost(ctx: AppContext) {
        ...
        const idAsUrl = parseURL(id);
        if (!idAsUrl) {
            return BadRequest('ID should be a valid URL');
        }
        ...
        // Check if the post is actually reposted by this account
        const isReposted = await this.postService.isRepostedByAccount(
            originalPost.id!,
            account.id,
        );

        if (!isReposted) {
            return Conflict('Post is not reposted');
        }

        originalPost.removeRepost(account);
        await this.postRepository.save(originalPost);

        return new Response(JSON.stringify({ dereposted: true }), {
            headers: { 'Content-Type': 'application/json' },
            status: 200,
        });
    }
