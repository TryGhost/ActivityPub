...
async function setupPubSub(logging: Logger) {
    if (process.env.USE_MQ === 'true') {
        ...
        try {
            ...
            queue.registerErrorListener((error) =>
...
async function setupFedify(
    ...
    fedifyKv = await KnexKvStore.create(client, 'key_value');
    ...
    inboxListener
        ...
        .on(
            Accept,
            ensureCorrectContext(
                spanWrapper(createAcceptHandler(accountService)),
            ),
        )
        .onError(inboxErrorHandler)
        .on(
...
): Hono<{ Variables: HonoContextVariables }> {
...
let fedifyKv: KvStore;
export let fedify: ReturnType<typeof createFederation<ContextData>>;

async function main() {
    await setupObservability();
    const gcpStorageService = await setupStorage(logging);
    await setupPubSub(logging);
