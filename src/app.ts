...
if (queue instanceof GCloudPubSubPushMessageQueue) {
    const mqHandler = createPushMessageHandler(queue, globalLogging);

    app.post('/.ghost/activitypub/mq', spanWrapper(mqHandler));
    app.post('/.ghost/activitypub/pubsub/fedify/push', spanWrapper(mqHandler));
}
