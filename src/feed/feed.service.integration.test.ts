...
describe('FeedService', () => {
    ...
    describe('removePostFromFeeds', () => {
        it('should remove post from feeds of specified users', async () => {
            const feedService = new FeedService(client);
            ...
            // Create and add post to feeds
            const post = await createPost(followedAccount, {
                audience: Audience.Public,
            });
            await postRepository.save(post);
            await feedService.addPostToFeeds(post as PublicPost);

            // Verify post is in feeds initially
            ...
            await feedService.removePostFromFeeds(post as PublicPost);
