...
import { AccountAliasedEvent } from '@/account/events/account-aliased.event';

describe('AccountAliasedEvent', () => {
    describe('getName', () => {
        it('should return the event name from static method', () => {
            expect(AccountAliasedEvent.getName()).toBe('account.aliased');
        });

        it('should return the event name from instance method', () => {
            const event = new AccountAliasedEvent(
                123,
                new URL('https://mastodon.social/users/alice'),
            );

            expect(event.getName()).toBe('account.aliased');
        });
    });
    ...
    describe('getAliasApId', () => {
        it('should return the alias AP ID', () => {
            ...
            const event = new AccountAliasedEvent(123, aliasApId);
    ...
    describe('fromJSON', () => {
        it('should deserialize the event from JSON', () => {
            const event = AccountAliasedEvent.fromJSON({
                accountId: 123,
                aliasApId: 'https://mastodon.social/users/alice',
            });
    ...
    describe('round-trip serialization', () => {
        it('should correctly serialize and deserialize', () => {
            const original = new AccountAliasedEvent(
                999,
                new URL('https://mastodon.social/users/alice'),
            );
            const json = original.toJSON();
            const restored = AccountAliasedEvent.fromJSON(json);
