...
describe('KnexPostRepository', () => {
    ...
    it('Handles likes of a new post', async () => {
        ...
        expect(eventsEmitSpy).nthCalledWith(
            ...
            new PostLikedEvent(Number(post.id), Number(accounts[0].id)),
        );
        expect(eventsEmitSpy).nthCalledWith(
            3,
            PostLikedEvent.getName(),
            new PostLikedEvent(Number(post.id), Number(accounts[1].id)),
        );
        expect(eventsEmitSpy).nthCalledWith(
            4,
            PostLikedEvent.getName(),
            new PostLikedEvent(Number(post.id), Number(accounts[2].id)),
        );
    });
